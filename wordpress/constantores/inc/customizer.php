...
function constantores_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'theme_bg_color',
			array(
				'label'		=> __( 'Header and footer background color', 'constantores'),
				'section'	=> 'colors',
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control(
			...
			'interactive_color', array(
				'label'		=> __( 'Interactive color (links etc)', 'constantores' ),
				'section'	=> 'colors',
	...
	$wp_customize->add_section( 'theme_options',
		array(
			'title'			=> __( 'Theme Options', 'constantores' ),
			'priority'		=> 95,
			'capability'	=> 'edit_theme_options',
			'description'	=> __( 'Change how much of a post is displayed on index and archive pages.', 'constantores' )
		)
	);
	...
	$wp_customize->add_setting(	'length_setting',
		array(
			'default'			=> 'excerpt',
			'type'				=> 'theme_mod',
			'sanitize_callback' => 'constantores_sanitize_length', // Sanitization function appears further down
			'transport'			=> 'postMessage'
		)
	);

	// Add the controls
	$wp_customize->add_control(	'constantores_length_control',
		array(
			'type'		=> 'radio',
			'label'		=> __( 'Index/archive displays', 'constantores' ),
			'section'	=> 'theme_options',
			'choices'	=> array(
				'excerpt'		=> __( 'Excerpt (default)', 'constantores' ),
				'full-content'	=> __( 'Full content', 'constantores' )
			),
...
function constantores_sanitize_length( $value ) {
...
function constantores_header_style() {
	...
	if ( '#b51c35' != $interactive_color ) { ?>
		<style type="text/css">
			...
			.cat-links a,
			.social-menu ul a,
			.widget_constantores_recent_posts a:hover .title,
			.widget_constantores_recent_posts a:focus .title,
			.widget_constantores_recent_comments a:focus .original-title,
			.widget_constantores_recent_comments a:hover .original-title,
			.pagination .current {
